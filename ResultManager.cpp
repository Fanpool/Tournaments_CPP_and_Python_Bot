...
void ResultManager::addResult(const std::string &discipline, const std::string &firstName, const std::string &lastName,
    ...
    if (!exists) {
        PGresult *resInsert = SQLRequests::addResultRequest(discipline, firstName, lastName, year_of_birth, point);
