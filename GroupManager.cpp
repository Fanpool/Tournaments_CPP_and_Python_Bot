...
void GroupManager::printGroup(PGresult *_command, PGresult *res, int groupId) {
    ...
    std::cout << "Год рождения: " << PQgetvalue(_command, groupId - 1, 0) << std::endl;
...
void GroupManager::addParticipantsToGroup(const std::vector<int> &participants, int groupId) {
...
void GroupManager::sortParticipantsIntoGroups() {
    ...
    for (const auto &year: years) {
        ...
        addParticipantsToGroup(participants, groupId);
    }
}
